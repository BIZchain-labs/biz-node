...
import (
	...
	"github.com/BIZchain-labs/biz-node/common/prque"
	...
	"github.com/BIZchain-labs/biz-node/core/state/snapshot"
	...
	"github.com/BIZchain-labs/biz-node/internal/syncx"
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		//
		// But if the `TxIndexTail` is not nil, e.g. Biz is initialized with
...
// sync, Biz will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init Biz with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded Biz to a new version which supports transaction
