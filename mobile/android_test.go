...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Biz.LightScryptN, Biz.LightScryptP);
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "biz-android-")
	if err != nil {
	...
	// Generate the mobile bindings for Biz and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/BIZchain-labs/biz-node/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "biz.aar"), "biz.aar")
