...
// Package gethclient provides an RPC client for biz-specific APIs.
package gethclient
...
// Client is a wrapper around rpc.Client that implements biz-specific functionality.
//
...
// GCStats retrieves the current garbage collection stats from a biz node.
...
// MemStats retrieves the current memory stats from a biz node.
...
// GetNodeInfo retrieves the node info of a biz node.
