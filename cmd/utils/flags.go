...
import (
	...
	"github.com/BIZchain-labs/biz-node/common/fdlimit"
	...
	"github.com/BIZchain-labs/biz-node/eth"
	...
	"github.com/BIZchain-labs/biz-node/graphql"
	...
	"github.com/BIZchain-labs/biz-node/metrics/exp"
	"github.com/BIZchain-labs/biz-node/metrics/influxdb"
	...
	"github.com/BIZchain-labs/biz-node/p2p/netutil"
...
// for Biz and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `biz account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	case ctx.GlobalBool(MainnetFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 707070
		}
		cfg.Genesis = core.DefaultGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.MainnetGenesisHash)
	case ctx.GlobalBool(TestnetFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 808080
		}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "biz.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.GlobalString(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "biz.", tagsMap)
		}
...
// new format.
//
// e.g. biz account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// biz --keystore /tmp/mykeystore --lightkdf account new
//
